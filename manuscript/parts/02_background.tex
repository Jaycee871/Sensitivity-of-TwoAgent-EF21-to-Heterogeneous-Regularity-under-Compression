...
HAC Law corresponds to Empirical Law 4.3 of Berg Thomsen, Taylor, and Dieuleveut for the heterogeneous EF$^{21}$ setting with $n=2$ \citep{thomsen2026tight}. The two workers may have different local smoothness constants $L_i$ and local strong-convexity constants $\mu_i$, while communication is governed by a contractive compression level $\epsilon\in(0,1)$.
...
The source paper supports HAC Law through numerical Performance
