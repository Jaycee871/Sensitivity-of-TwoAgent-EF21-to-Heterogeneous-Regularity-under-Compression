...
The independent reproduction grid follows the two-agent parameter sweep used for Empirical Law 4.3. For each worker, the local smoothness constant is selected from
\[
L_i\in\{1,10,100,1000\},
\]
and the local condition number is selected from
\[
\kappa_i\in\{2,10,100,1000,10000\}.
\]
This gives 20 possible local regularity configurations per worker. Because the two-agent relation is invariant to exchanging the worker labels, permutation-equivalent worker pairs are counted only once. The resulting two-worker grid therefore contains
\[
\binom{20+2-1}{2}=\binom{21}{2}=210
\]
distinct regularity configurations. Each of these 210 configurations is then evaluated at 50 equally spaced compression levels,
\[
\epsilon\in[0.05,0.95],
\]
yielding a total of
\[
210\times50=10{,}500
\]
parameter configurations. For each worker, the corresponding strong-convexity constant is obtained from $\mu_i=L_i/\kappa_i$. Before evaluating the cubic, the local smoothness constants are normalized by the largest $L_i$ in the worker pair, with the same scaling applied to the corresponding $\mu_i$. This normalization preserves the relevant condition numbers while avoiding redundant overall scale variation.
